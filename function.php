<?php 
include('define.php');
///// bloque les chmaps etr caracteres spéciaux
function form_champs_bloque()
{
echo "<td><div class='clear'><div class='input-box'><textarea class='message' name='tabsaisies[]' id='val[]' onblur='verifChamp(this)'onKeypress='if(event.keyCode==39||  event.keyCode==33  ||  event.keyCode==34||  event.keyCode==35 || event.keyCode==36|| event.keyCode==38 || event.keyCode==47 || event.keyCode==123 || event.keyCode==125) event.returnValue = false;if (event.which==39||  event.which==33  ||  event.which==34||  event.which==35 || event.which==36|| event.which==38 || event.which==47 || event.which==123 || event.which==125) return false;'></textarea></div></div><td>";
}
